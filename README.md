...
For my first cybersecurity project, I analyzed 5 real-world phishing samples from PhishTank and my own research. Each sample is broken down by red flags, social engineering tactics, and what would actually happen if someone fell for it. I attached the formal report in this repo as well.
...
The long random string of characters in the subdomain is a giveaway of phishing infrastructure
...
The attacker hosted the page on Cloudflare's infrastructure, which automatically gives it a valid HTTPS certificate. The padlock shows up in the browser, which many users take as a sign the site is safe
...
internacionall has a double l, which is classic typosquatting. Easy to miss at a glance
...
Includes a phone number, address, and a privacy policy, which things people associate with legitimate businesses but rarely verify
...
The sender address is not a @berkeley.edu email, which is the clearest single red flag
...
The email is well-written and professionally structured. It is much higher quality than typical phishing
...
Social engineering did most of the heavy lifting. Urgency and fear were used to push users into critical thinking.
HTTPS doesn't mean safe. Sample 2 was the clearest example of this. The attacker got a valid SSL certificate just by hosting on Cloudflare. The padlock showed up. The site was still malicious. 
The URL is almost always the giveaway. Across every URL-based sample, the base domain had nothing to do with the organization being impersonated. Learning to read a URL carefully — especially the base domain before any path or parameters.
...
The Cloudflare infrastructure abuse in Sample 2 changed how I think about HTTPS. I went into this assuming a padlock meant something. Attackers can get valid SSL certificates just by hosting content on legitimate cloud platforms. That was probably the most practically useful thing I took away.
Typosquatting is sneaky. The internacionall domain in Sample 3 looks fine at a normal reading speed. You have to slow down and actually read the characters.
The UC Berkeley spear phish reminded me that the most dangerous attacks aren't always the most technical ones. A well-written email targeting the right person in the right context can be more effective than sophisticated malware.
Overall this project made abstract concepts like indicators of compromise and social engineering feel interesting and fun. Looking forward to building on this in future projects.
